<script lang="ts">
	...
	import type { GeneralCreate } from '$lib/forms';
	...
	use:enhance={() => {
		return async ({ result, update }) => {
			if (result.type == 'success') {
				...
				if (index < topic.generalQuestions.length - 1) {
					index++;
				} else {
					index = 0;
				}
		...
		<div class="bt-group">
			<button
				onclick={(event) => {
					...
					const answerInput = document.getElementsByClassName('answer')[1] as HTMLInputElement;
					answerInput.focus();
					if (index < topic.generalQuestions!.length - 1) {
						index++;
						answer = '';
			...
			>
			<span class="number">{index + 1}/{topic.generalQuestions.length}</span>
		</div>
	</div>
	<div>
		<span>Question</span>
		{#key index}
			<span class="question" in:typewriter={{ speed: 5 }}>{topic.generalQuestions[index]}</span>
		{/key}
		<input type="hidden" name="question" value={topic.generalQuestions[index]} />
	</div>
